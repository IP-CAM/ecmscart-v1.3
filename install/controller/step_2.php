...
class ControllerStep2 extends Controller {
	...
	public function index() {
		...
		$data['config_catalog'] = DIR_ECMSCART . 'config.php';
		$data['config_admin'] = DIR_ECMSCART . 'admin/config.php';

		$data['cache'] = DIR_SYSTEM . 'cache';
		$data['logs'] = DIR_SYSTEM . 'logs';
		$data['download'] = DIR_SYSTEM . 'download';
		$data['upload'] = DIR_SYSTEM . 'upload';
		$data['image'] = DIR_ECMSCART . 'image';
		$data['image_cache'] = DIR_ECMSCART . 'image/cache';
		$data['image_data'] = DIR_ECMSCART . 'image/catalog';
	...
	private function validate() {
		if (phpversion() < '5.3') {
			$this->error['warning'] = 'Warning: You need to use PHP5.3 or above for eCmsCart to work!';
		}
		...
		if (ini_get('session.auto_start')) {
			$this->error['warning'] = 'Warning: eCmsCart will not work with session.auto_start enabled!';
		}

		if (!array_filter(array('mysql', 'mysqli', 'pdo', 'pgsql'), 'extension_loaded')) {
			$this->error['warning'] = 'Warning: A database extension needs to be loaded in the php.ini for eCmsCart to work!';
		}

		if (!extension_loaded('gd')) {
			$this->error['warning'] = 'Warning: GD extension needs to be loaded for eCmsCart to work!';
		}

		if (!extension_loaded('curl')) {
			$this->error['warning'] = 'Warning: CURL extension needs to be loaded for eCmsCart to work!';
		}

		if (!function_exists('mcrypt_encrypt')) {
			$this->error['warning'] = 'Warning: mCrypt extension needs to be loaded for eCmsCart to work!';
		}

		if (!extension_loaded('zlib')) {
			$this->error['warning'] = 'Warning: ZLIB extension needs to be loaded for eCmsCart to work!';
		}

		if (!extension_loaded('zip')) {
			$this->error['warning'] = 'Warning: ZIP extension needs to be loaded for eCmsCart to work!';
		}

		if (!function_exists('iconv')) {
			if (!extension_loaded('mbstring')) {
				$this->error['warning'] = 'Warning: mbstring extension needs to be loaded for eCmsCart to work!';
			}
		}

		if (!file_exists(DIR_ECMSCART . 'config.php')) {
			$this->error['warning'] = 'Warning: config.php does not exist. You need to rename config-dist.php to config.php!';
		} elseif (!is_writable(DIR_ECMSCART . 'config.php')) {
			$this->error['warning'] = 'Warning: config.php needs to be writable for eCmsCart to be installed!';
		}

		if (!file_exists(DIR_ECMSCART . 'admin/config.php')) {
			$this->error['warning'] = 'Warning: admin/config.php does not exist. You need to rename admin/config-dist.php to admin/config.php!';
		} elseif (!is_writable(DIR_ECMSCART . 'admin/config.php')) {
			$this->error['warning'] = 'Warning: admin/config.php needs to be writable for eCmsCart to be installed!';
		}

		if (!is_writable(DIR_SYSTEM . 'cache')) {
			$this->error['warning'] = 'Warning: Cache directory needs to be writable for eCmsCart to work!';
		}

		if (!is_writable(DIR_SYSTEM . 'logs')) {
			$this->error['warning'] = 'Warning: Logs directory needs to be writable for eCmsCart to work!';
		}

		if (!is_writable(DIR_SYSTEM . 'download')) {
			$this->error['warning'] = 'Warning: Download directory needs to be writable for eCmsCart to work!';
		}
		
		if (!is_writable(DIR_SYSTEM . 'upload')) {
			$this->error['warning'] = 'Warning: Upload directory needs to be writable for eCmsCart to work!';
		}
		
		if (!is_writable(DIR_ECMSCART . 'image')) {
			$this->error['warning'] = 'Warning: Image directory needs to be writable for eCmsCart to work!';
		}

		if (!is_writable(DIR_ECMSCART . 'image/cache')) {
			$this->error['warning'] = 'Warning: Image cache directory needs to be writable for eCmsCart to work!';
		}

		if (!is_writable(DIR_ECMSCART . 'image/catalog')) {
			$this->error['warning'] = 'Warning: Image catalog directory needs to be writable for eCmsCart to work!';
		}
