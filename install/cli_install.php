...
//                               --db_username root \
//                               --db_password pass \
//                               --db_database ecmscart \
//                               --db_driver mysqli \
//                               --username admin \
//                               --password admin \
//                               --email youremail@example.com \
//                               --http_server http://localhost/ecmscart
//
...
define('DIR_ECMSCART', str_replace('\'', '/', realpath(DIR_APPLICATION . '../')) . '/');
...
function check_requirements() {
	$error = null;
	if (phpversion() < '5.0') {
		$error = 'Warning: You need to use PHP5 or above for eCmsCart to work!';
	}
	...
	if (ini_get('session.auto_start')) {
		$error = 'Warning: eCmsCart will not work with session.auto_start enabled!';
	}

	if (!extension_loaded('mysqli')) {
		$error = 'Warning: MySQLi extension needs to be loaded for eCmsCart to work!';
	}

	if (!extension_loaded('gd')) {
		$error = 'Warning: GD extension needs to be loaded for eCmsCart to work!';
	}

	if (!extension_loaded('curl')) {
		$error = 'Warning: CURL extension needs to be loaded for eCmsCart to work!';
	}

	if (!function_exists('mcrypt_encrypt')) {
		$error = 'Warning: mCrypt extension needs to be loaded for eCmsCart to work!';
	}

	if (!extension_loaded('zlib')) {
		$error = 'Warning: ZLIB extension needs to be loaded for eCmsCart to work!';
	}
...
function write_config_files($options) {
	...
	$output .= '// DIR' . "\n";
	$output .= 'define(\'DIR_APPLICATION\', \'' . DIR_ECMSCART . 'catalog/\');' . "\n";
	$output .= 'define(\'DIR_SYSTEM\', \'' . DIR_ECMSCART . 'system/\');' . "\n";
	$output .= 'define(\'DIR_DATABASE\', \'' . DIR_ECMSCART . 'system/database/\');' . "\n";
	$output .= 'define(\'DIR_LANGUAGE\', \'' . DIR_ECMSCART . 'catalog/language/\');' . "\n";
	$output .= 'define(\'DIR_TEMPLATE\', \'' . DIR_ECMSCART . 'catalog/view/theme/\');' . "\n";
	$output .= 'define(\'DIR_CONFIG\', \'' . DIR_ECMSCART . 'system/config/\');' . "\n";
	$output .= 'define(\'DIR_IMAGE\', \'' . DIR_ECMSCART . 'image/\');' . "\n";
	$output .= 'define(\'DIR_CACHE\', \'' . DIR_ECMSCART . 'system/cache/\');' . "\n";
	$output .= 'define(\'DIR_DOWNLOAD\', \'' . DIR_ECMSCART . 'system/download/\');' . "\n";
	$output .= 'define(\'DIR_MODIFICATION\', \'' . DIR_ECMSCART . 'system/modification/\');' . "\n";
	$output .= 'define(\'DIR_LOGS\', \'' . DIR_ECMSCART . 'system/logs/\');' . "\n\n";
	...
	$file = fopen(DIR_ECMSCART . 'config.php', 'w');
	...
	$output .= '// DIR' . "\n";
	$output .= 'define(\'DIR_APPLICATION\', \'' . DIR_ECMSCART . 'admin/\');' . "\n";
	$output .= 'define(\'DIR_SYSTEM\', \'' . DIR_ECMSCART . 'system/\');' . "\n";
	$output .= 'define(\'DIR_DATABASE\', \'' . DIR_ECMSCART . 'system/database/\');' . "\n";
	$output .= 'define(\'DIR_LANGUAGE\', \'' . DIR_ECMSCART . 'admin/language/\');' . "\n";
	$output .= 'define(\'DIR_TEMPLATE\', \'' . DIR_ECMSCART . 'admin/view/template/\');' . "\n";
	$output .= 'define(\'DIR_CONFIG\', \'' . DIR_ECMSCART . 'system/config/\');' . "\n";
	$output .= 'define(\'DIR_IMAGE\', \'' . DIR_ECMSCART . 'image/\');' . "\n";
	$output .= 'define(\'DIR_CACHE\', \'' . DIR_ECMSCART . 'system/cache/\');' . "\n";
	$output .= 'define(\'DIR_DOWNLOAD\', \'' . DIR_ECMSCART . 'system/download/\');' . "\n";
	$output .= 'define(\'DIR_LOGS\', \'' . DIR_ECMSCART . 'system/logs/\');' . "\n";
	$output .= 'define(\'DIR_MODIFICATION\', \'' . DIR_ECMSCART . 'system/modification/\');' . "\n";
	$output .= 'define(\'DIR_CATALOG\', \'' . DIR_ECMSCART . 'catalog/\');' . "\n\n";
	...
	$file = fopen(DIR_ECMSCART . 'admin/config.php', 'w');
...
case "install":
	try {
		...
		define('HTTP_ECMSCART', $options['http_server']);
		$valid = valid($options);
		if (!$valid[0]) {
		...
		install($options);
		echo "SUCCESS! eCmsCart successfully installed on your server\n";
