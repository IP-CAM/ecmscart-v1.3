...
class ModelInstall extends Model {
	public function database($data) {
		...
		$file = DIR_APPLICATION . 'ecmscart.sql';
		...
		if ($lines) {
			...
			foreach($lines as $line) {
				if ($line && (substr($line, 0, 2) != '--') && (substr($line, 0, 1) != '#')) {
					...
					if (preg_match('/;\s*$/', $line)) {
						$sql = str_replace("DROP TABLE IF EXISTS `ec_", "DROP TABLE IF EXISTS `" . $data['db_prefix'], $sql);
						$sql = str_replace("CREATE TABLE IF NOT EXISTS `ec_", "CREATE TABLE IF NOT EXISTS `" . $data['db_prefix'], $sql);
						$sql = str_replace("INSERT INTO `ec_", "INSERT INTO `" . $data['db_prefix'], $sql);
			...
			$db->query("INSERT INTO `" . $data['db_prefix'] . "setting` SET `code` = 'config', `key` = 'config_url', value = '" . $db->escape(HTTP_ECMSCART) . "'");
