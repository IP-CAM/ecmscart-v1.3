...
class ControllerModuleCarousel extends Controller {
	...
	public function index() {
		$this->data = $this->load->language('module/carousel');
		...
		if (!isset($this->request->get['module_id'])) {// dyanamic adding module url for module_id
			$module_url = $this->url->link('module/carousel', 'token=' . $this->session->data['token'], 'SSL');
			
		} else {
			$module_url = $this->url->link('module/carousel', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');			
		}
		...
		if (!isset($this->request->get['module_id'])) {
			...
			$this->data['action'] = $this->url->link('module/carousel', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}
		...
		if (!empty($module_info) && !$this->error) {
			...
			$this->data['width'] = $this->request->post('width', 130);
		}	
			
		if (!empty($module_info) && !$this->error) {
			$this->data['height'] = $module_info['height'];
		} else {
			$this->data['height'] =  $this->request->post('height', 100);
		}	
		...
		$this->response->setOutput($this->load->view('module/carousel.tpl', $this->data));
	}
