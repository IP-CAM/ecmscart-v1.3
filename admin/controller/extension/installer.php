...
class ControllerExtensionInstaller extends Controller {
	...
	public function index() {
		$this->data = $this->load->language('extension/installer');
		...
		$this->data['breadcrumbs'] = $this->config->breadcrums(array(
							...
							$this->url->link('extension/installer', 'token=' . $this->session->data['token'], 'SSL')	// Link URL
						));
		...
		$this->data['error_warning'] = ($directories)? $this->data['error_temporary']: '';
		...
		$this->response->setOutput($this->load->view('extension/installer.tpl', $this->data));
	}
	...
	public function upload() {
		...
		if (!$json) {
			if (!empty($this->request->files['file']['name'])) {
				if (substr($this->request->files['file']['name'], -10) != '.vqmod.zip' && substr($this->request->files['file']['name'], -10) != '.vqmod.xml') {
			...
			if (strrchr($this->request->files['file']['name'], '.') == '.zip') {
				...
				if (file_exists($file)) {
					...
					if ($zip) {
						...
						$json['error'] = $this->data['error_unzip'];
					}
				} else {
					$json['error'] = $this->data['error_file'];
				}
	...
	public function ftp() {
		...
		if (!$this->config->get('config_ftp_status')) {
			$json['error'] = $this->data['error_ftp_status'];
		}

		$directory = DIR_UPLOAD . str_replace(array('../', '..\\', '..'), '', $this->request->post['path']) . '/upload/';

		if (!is_dir($directory)) {
			$json['error'] = $this->data['error_directory'];
		}
						...
						foreach ($files as $file) {
							...
							if (is_dir($file)) {
								...
								if (!in_array(basename($destination), $list_data)) {
									if (!ftp_mkdir($connection, $destination)) {
										$json['error'] = sprintf($this->data['error_ftp_directory'], $destination);
									}
								}
							}
							...
							if (is_file($file)) {
								if (!ftp_put($connection, $destination, $file, FTP_BINARY)) {
									$json['error'] = sprintf($this->data['error_ftp_file'], $file);
								}
							}
						}
					} else {
						$json['error'] = sprintf($this->data['error_ftp_root'], $root);
					}
				} else {
					$json['error'] = sprintf($this->data['error_ftp_login'], $this->config->get('config_ftp_username'));
				}

				ftp_close($connection);
			} else {
				$json['error'] = sprintf($this->data['error_ftp_connection'], $this->config->get('config_ftp_hostname'), $this->config->get('config_ftp_port'));
			}
		}
		...
		if (!$json) {
			...
			if ($lines) {
				try {
					...
					$json['error'] = sprintf($this->data['error_exception'], $exception->getCode(), $exception->getMessage(), $exception->getFile(), $exception->getLine());
				}
			}
		}
				...
				try {
					...
					if ($code) {
						...
						if ($modification_info) {
							$json['error'] = sprintf($this->data['error_exists'], $modification_info['name']);
						}
					} else {
						$json['error'] = $this->data['error_code'];
					}
	...
	public function clear() {
		...
		if (!$json) {
			...
			$json['success'] = $this->data['text_clear'];
		}
