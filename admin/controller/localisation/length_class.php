...
class ControllerLocalisationLengthClass extends Controller {
	...
	public function index() {
		$this->data = $this->load->language('localisation/length_class');
	...
	protected function getList() {
		...
		$this->data['breadcrumbs'] = $this->config->breadcrums(array(
							...
							$this->url->link('localisation/length_class', 'token=' . $this->session->data['token'] . $url, 'SSL')	// Link URL
						));
						
		$this->data['save'] = $this->url->link('localisation/length_class/save', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		foreach ($results as $result) {
			$this->data['length_classes'][] = array(
				...
				'title'           => $result['title'] . (($result['length_class_id'] == $this->config->get('config_length_class_id')) ? $this->data['text_default'] : null),
				'unit'            => $result['unit'],
				'value'           => $result['value'],
				'save'            => $this->url->link('localisation/length_class/save', 'token=' . $this->session->data['token'] . '&length_class_id=' . $result['length_class_id'] . $url, 'SSL')
			);
		}
		...
		$this->data['results'] = sprintf($this->data['text_pagination'], ($length_class_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_limit_admin')) > ($length_class_total - $this->config->get('config_limit_admin'))) ? $length_class_total : ((($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')), $length_class_total, ceil($length_class_total / $this->config->get('config_limit_admin')));
		...
		$this->response->setOutput($this->load->view('localisation/length_class_list.tpl', $this->data));
	}

	protected function getForm() {		
		$this->data['text_form'] = !isset($this->request->get['length_class_id']) ? $this->data['text_add'] : $this->data['text_edit'];
		
		$this->data['error_warning'] =  (isset($this->error['warning'])? $this->error['warning']: '');
		
		$this->data['error_title'] =  (isset($this->error['title'])? $this->error['title']: array());
		
		$this->data['error_unit'] =  (isset($this->error['unit'])? $this->error['unit']: array());
		...
		if (!isset($this->request->get['length_class_id'])) {
			$this->data['action'] = $this->url->link('localisation/length_class/save', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$this->data['action'] = $this->url->link('localisation/length_class/save', 'token=' . $this->session->data['token'] . '&length_class_id=' . $this->request->get['length_class_id'] . $url, 'SSL');
		}
		...
		if (isset($this->request->get['length_class_id']) && !$this->error) {
			...
			$this->data['length_class_description'] = $this->request->post('length_class_description', array());
		}

		if (!empty($length_class_info) && !$this->error) {
		...
		$this->response->setOutput($this->load->view('localisation/length_class_form.tpl', $this->data));
	}
