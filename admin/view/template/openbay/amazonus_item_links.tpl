    ...
    <form id="product-form">
        ...
        <table class="table table-bordered table-hover">
          ...
          <tr>
            ...
            <td>
              <input id="new-amazonus-sku" type="text" class="form-control" autocomplete="off">
            </td>
...
function loadLinks() {
  $.ajax({
    ...
    type: 'get',
    dataType: 'json',
    data: 'product_id=' + encodeURIComponent($('#new-product-id').val()) + '&amazonus_sku=' + encodeURIComponent($('#new-amazonus-sku').val()),
  ...
  $.ajax({
    ...
    success: function (json) {
      ...
      for (i in json) {
        ...
        rows += '<div class="amazonus_sku_div_' + json[i]['product_id'] + '_' + json[i]['var'] + '">';
        rows += '<div class="row">';
        rows += '<div class="col-sm-8 form-group">';
        rows += '<input class="form-control amazonus_sku_' + json[i]['product_id'] + '_' + json[i]['var'] + '"  type="text">';
        rows += '</div>';
...
function addNewSkuField(product_id, variation) {
  var html = '';
  html += '<div class="amazonus_sku_div_' + product_id + '_' + variation + '">';
  html += '<div class="row">';
  html += '<div class="col-sm-8 form-group">';
  html += '<input class="form-control amazonus_sku_' + product_id + '_' + variation + '"  type="text">';
  html += '</div>';
  ...
  $(".amazonus_sku_div_" + product_id + "_" + variation.replace(":", "\\:")).last().after(html);
}
...
function removeSkuField(icon, product_id, variation) {
  ...
  $(".amazonus_sku_div_" + product_id + "_" + variation.replace(':', '\\:') + ":eq(" + removeIndex + ")").remove();
}

function addNewLink(button, product_id, variation) {
  var errors = 0;

  console.log(".amazonus_sku_" + product_id + "_" + variation.replace(':', '\\:'));
  $(".amazonus_sku_" + product_id + "_" + variation.replace(':', '\\:')).each(function (index) {
    if ($(this).val() == '') {
      errors++;
    }
  });
...
function addNewLinkAutocomplete() {
  ...
  if ($('#new-amazonus-sku').val() == "") {
    alert('<?php echo $error_empty_sku; ?>');
    return;
  }

  var product_id = $('#new-product-id').val();
  var amazonus_sku = $('#new-amazonus-sku').val();
  var variation = '';
  if ($('#openstock-option-selector').length != 0) {
    variation = $('#openstock-option-selector').val();
  }

  addLink('#add-new-button', product_id, amazonus_sku, variation);

  $('#new-product').val('');
  $('#new-amazonus-sku').val('');
  $('#new-product-id').val('');
