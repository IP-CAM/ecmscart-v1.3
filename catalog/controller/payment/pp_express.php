...
class ControllerPaymentPPExpress extends Controller {
	public function index() {
		$this->data = $this->language->load('payment/pp_express');
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/pp_express.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/payment/pp_express.tpl', $this->data);
		} else {
			return $this->load->view('default/template/payment/pp_express.tpl', $this->data);
		}
	}
	...
	public function express() {
		...
		if (isset($this->session->data['pp_login']['seamless']['access_token']) && (isset($this->session->data['pp_login']['seamless']['customer_id']) && $this->session->data['pp_login']['seamless']['customer_id'] == $this->customer->getId()) && $this->config->get('pp_login_seamless')) {
			$this->data['IDENTITYACCESSTOKEN'] = $this->session->data['pp_login']['seamless']['access_token'];
		}

		$this->data = array_merge($this->data, $this->model_payment_pp_express->paymentRequestInfo());

		$result = $this->model_payment_pp_express->call($this->data);
	...
	public function expressConfirm() {
		...
		if (isset($this->request->post['coupon']) && $this->validateCoupon()) {
			...
			$this->session->data['success'] = $this->data['text_coupon'];
		...
		if (isset($this->request->post['voucher']) && $this->validateVoucher()) {
			...
			$this->session->data['success'] = $this->data['text_voucher'];
		...
		if (isset($this->request->post['reward']) && $this->validateReward()) {
			...
			$this->session->data['success'] = $this->data['text_reward'];

			$this->response->redirect($this->url->link('payment/pp_express/expressConfirm', '', 'SSL'));
		}

		$this->document->setTitle($this->data['express_text_title']);

		$this->data['heading_title'] = $this->data['express_text_title'];
		
		$this->data['breadcrumbs'] = $this->config->breadcrums(array(
							 $this->data['text_home'],
							$this->url->link('common/home'),
							$this->data['text_title'],	// Text to display link
							$this->url->link('payment/pp_express/express'),
							$this->data['express_text_title'],
							$this->url->link('payment/pp_express/expressConfirm')
						));
		...
		foreach ($products as $product) {
			...
			if ($product['recurring']) {
				...
				if ($product['recurring']['trial']) {
					...
					$recurring_description = sprintf($this->data['text_trial_description'], $recurring_price, $product['recurring']['trial_cycle'], $frequencies[$product['recurring']['trial_frequency']], $product['recurring']['trial_duration']) . ' ';
				}
				...
				if ($product['recurring']['duration']) {
					$recurring_description .= sprintf($this->data['text_payment_description'], $recurring_price, $product['recurring']['cycle'], $frequencies[$product['recurring']['frequency']], $product['recurring']['duration']);
				} else {
					$recurring_description .= sprintf($this->data['text_payment_cancel'], $recurring_price, $product['recurring']['cycle'], $frequencies[$product['recurring']['frequency']], $product['recurring']['duration']);
				}
			}
			...
			if (!empty($shipping_address)) {
				...
				if (!empty($results)) {
					...
					if (!empty($quote_data)) {
						...
						unset($this->session->data['shipping_methods']);
						unset($this->session->data['shipping_method']);
						$this->data['error_no_shipping'] = $this->data['error_no_shipping'];
					}
				} else {
					unset($this->session->data['shipping_methods']);
					unset($this->session->data['shipping_method']);
		...
		$this->data['error_warning'] =  (isset($this->session->data['error_warning'])? $this->session->data['error_warning']: '');
		
		if (isset($this->session->data['error_warning']))  // To unset success session variable.
		...
		$this->data['attention'] =  (isset($this->session->data['attention'])? $this->session->data['attention']: '');
		
		if (isset($this->session->data['attention']))  // To unset success session variable.
			unset($this->session->data['attention']);
		
		$this->data['coupon'] = $this->load->controller('module/coupon');
		$this->data['voucher'] = $this->load->controller('module/voucher');
		$this->data['reward'] = $this->load->controller('module/reward');
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/pp_express_confirm.tpl')) {
			$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/payment/pp_express_confirm.tpl', $this->data));
		} else {
			$this->response->setOutput($this->load->view('default/template/payment/pp_express_confirm.tpl', $this->data));
		}
	}
	...
	public function expressComplete() {
		...
		if ($redirect == '') {
			...
			$this->data['payment_firstname'] = isset($payment_address['firstname']) ? $payment_address['firstname'] : '';
			$this->data['payment_lastname'] = isset($payment_address['lastname']) ? $payment_address['lastname'] : '';
			$this->data['payment_company'] = isset($payment_address['company']) ? $payment_address['company'] : '';
			$this->data['payment_company_id'] = isset($payment_address['company_id']) ? $payment_address['company_id'] : '';
			$this->data['payment_tax_id'] = isset($payment_address['tax_id']) ? $payment_address['tax_id'] : '';
			$this->data['payment_address_1'] = isset($payment_address['address_1']) ? $payment_address['address_1'] : '';
			$this->data['payment_address_2'] = isset($payment_address['address_2']) ? $payment_address['address_2'] : '';
			$this->data['payment_city'] = isset($payment_address['city']) ? $payment_address['city'] : '';
			$this->data['payment_postcode'] = isset($payment_address['postcode']) ? $payment_address['postcode'] : '';
			$this->data['payment_zone'] = isset($payment_address['zone']) ? $payment_address['zone'] : '';
			$this->data['payment_zone_id'] = isset($payment_address['zone_id']) ? $payment_address['zone_id'] : '';
			$this->data['payment_country'] = isset($payment_address['country']) ? $payment_address['country'] : '';
			$this->data['payment_country_id'] = isset($payment_address['country_id']) ? $payment_address['country_id'] : '';
			$this->data['payment_address_format'] = isset($payment_address['address_format']) ? $payment_address['address_format'] : '';
			...
			if ($this->cart->hasShipping()) {
				...
				$this->data['shipping_zone'] = $shipping_address['zone'];
				$this->data['shipping_zone_id'] = $shipping_address['zone_id'];
				$this->data['shipping_country'] = $shipping_address['country'];
				$this->data['shipping_country_id'] = $shipping_address['country_id'];
				$this->data['shipping_address_format'] = $shipping_address['address_format'];
				...
				$this->data['shipping_address_format'] = '';
				$this->data['shipping_method'] = '';
				$this->data['shipping_code'] = '';
			}
			...
			if (isset($this->request->cookie['tracking'])) {
				$this->data['tracking'] = $this->request->cookie['tracking'];
				...
				if ($affiliate_info) {
					$this->data['affiliate_id'] = $affiliate_info['affiliate_id'];
					$this->data['commission'] = ($subtotal / 100) * $affiliate_info['commission'];
				} else {
				...
				if ($marketing_info) {
					$this->data['marketing_id'] = $marketing_info['marketing_id'];
				} else {
					$this->data['marketing_id'] = 0;
				}
			} else {
				$this->data['affiliate_id'] = 0;
				$this->data['commission'] = 0;
			...
			$this->data['currency_value'] = $this->currency->getValue($this->currency->getCode());
			$this->data['ip'] = $this->request->server['REMOTE_ADDR'];

			if (!empty($this->request->server['HTTP_X_FORWARDED_FOR'])) {
				$this->data['forwarded_ip'] = $this->request->server['HTTP_X_FORWARDED_FOR'];
			} elseif (!empty($this->request->server['HTTP_CLIENT_IP'])) {
				$this->data['forwarded_ip'] = $this->request->server['HTTP_CLIENT_IP'];
			} else {
				$this->data['forwarded_ip'] = '';
			}

			if (isset($this->request->server['HTTP_USER_AGENT'])) {
				$this->data['user_agent'] = $this->request->server['HTTP_USER_AGENT'];
			} else {
				$this->data['user_agent'] = '';
			}

			if (isset($this->request->server['HTTP_ACCEPT_LANGUAGE'])) {
				$this->data['accept_language'] = $this->request->server['HTTP_ACCEPT_LANGUAGE'];
			} else {
				$this->data['accept_language'] = '';
			}

			$this->load->model('account/custom_field');
			$this->load->model('checkout/order');

			$order_id = $this->model_checkout_order->addOrder($this->data);
				...
				if ($recurring_products) {
					...
					foreach($recurring_products as $item) {
						...
						if ($item['recurring']['trial']) {
							...
							$trial_text =  sprintf($this->data['text_trial'], $trial_amt, $item['recurring']['trial_cycle'], $item['recurring']['trial_frequency'], $item['recurring']['trial_duration']);

							$this->data = array_merge($this->data, $this->data_trial);
						} else {
							$trial_text = '';
						}
						...
						$recurring_description = $trial_text . sprintf($this->data['text_recurring'], $recurring_amt, $item['recurring']['cycle'], $item['recurring']['frequency']);

						if ($item['recurring']['duration'] > 0) {
							$recurring_description .= sprintf($this->data['text_length'], $item['recurring']['duration']);
						}
						...
						$this->data['PROFILEREFERENCE'] = $recurring_id;
						$this->data['DESC'] = $recurring_description;
				...
				if ($result['L_ERRORCODE0'] == '10486') {
					if (isset($this->session->data['paypal_redirect_count'])) {
						...
						if ($this->session->data['paypal_redirect_count'] == 2) {
							...
							$this->session->data['error'] = $this->data['error_too_many_failures'];
							$this->response->redirect($this->url->link('checkout/checkout', '', 'SSL'));
						} else {
	...
	public function checkout() {
		...
		$this->data = array_merge($this->data, $this->data_shipping);
	...
	public function checkoutReturn() {
		...
		if ($result['ACK'] == 'Success') {
			...
			$this->data['breadcrumbs'] = $this->config->breadcrums(array(
							...
							$this->url->link('common/home'),
							$this->data['text_cart'],	// Text to display link
							$this->url->link('checkout/cart')
						));

			$this->data['heading_title'] = $this->data['error_heading_title'];
	...
	protected function shippingValidate($code) {
		...
		if (empty($code)) {
			$this->session->data['error_warning'] = $this->data['error_shipping'];
			return false;
		} else {
			...
			if (!isset($shipping[0]) || !isset($shipping[1]) || !isset($this->session->data['shipping_methods'][$shipping[0]]['quote'][$shipping[1]])) {
				...
				$this->session->data['success'] = $this->data['text_shipping_updated'];
				return true;
			}
		}
	}
	...
	protected function validateVoucher() {
		...
		if (!$voucher_info) {
			$error = $this->data['error_voucher'];
		}

		if (!$error) {
			return true;
		} else {
			$this->session->data['error_warning'] = $this->data['error_voucher'];
			return false;
		}
	}
	...
	protected function validateReward() {
		...
		if (empty($this->request->post['reward'])) {
			$error = $this->data['error_reward'];
		}

		if ($this->request->post['reward'] > $points) {
			$error = sprintf($this->data['error_points'], $this->request->post['reward']);
		}

		if ($this->request->post['reward'] > $points_total) {
			$error = sprintf($this->data['error_maximum'], $points_total);
		}
	...
	public function recurringButtons() {
		...
		$this->data['buttons'][] = array(
			'text' => $this->data['button_continue'],
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/common/buttons.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/common/buttons.tpl', $this->data);
		} else {
			return $this->load->view('default/template/common/buttons.tpl', $this->data);
		}
	}
}
