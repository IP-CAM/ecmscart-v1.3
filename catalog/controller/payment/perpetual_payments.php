...
class ControllerPaymentPerpetualPayments extends Controller {
	public function index() {
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/perpetual_payments.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/payment/perpetual_payments.tpl', $this->data);
		} else {
			return $this->load->view('default/template/payment/perpetual_payments.tpl', $this->data);
		}
	}
	...
	public function send() {
		...
		if ($response) {
			$this->data = explode('|', $response);

			if (isset($this->data[0]) && $this->data[0] == 'A') {
				$message = '';

				if (isset($this->data[1])) {
					$message .= $this->data['text_transaction'] . ' ' . $this->data[1] . "\n";
				}

				if (isset($this->data[2])) {
					if ($this->data[2] == '232') {
						$message .= $this->data['text_avs'] . ' ' . $this->data['text_avs_full_match'] . "\n";
					} elseif ($this->data[2] == '400') {
						$message .= $this->data['text_avs'] . ' ' . $this->data['text_avs_not_match'] . "\n";
					}
				}

				if (isset($this->data[3])) {
					$message .= $this->data['text_authorisation'] . ' ' . $this->data[3] . "\n";
				}
