...
class ControllerPaymentAuthorizeNetAim extends Controller {
	public function index() {
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/authorizenet_aim.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/payment/authorizenet_aim.tpl', $this->data);
		} else {
			return $this->load->view('default/template/payment/authorizenet_aim.tpl', $this->data);
		}
	}
	...
	public function send() {
		...
		$this->data['x_login'] = $this->config->get('authorizenet_aim_login');
		$this->data['x_tran_key'] = $this->config->get('authorizenet_aim_key');
		...
		$this->data['x_address'] = html_entity_decode($order_info['payment_address_1'], ENT_QUOTES, 'UTF-8');
		...
		$this->data['x_amount'] = $this->currency->format($order_info['total'], $order_info['currency_code'], 1.00000, false);
		$this->data['x_currency_code'] = $this->currency->getCode();
		$this->data['x_method'] = 'CC';
		$this->data['x_type'] = ($this->config->get('authorizenet_aim_method') == 'capture') ? 'AUTH_CAPTURE' : 'AUTH_ONLY';
		$this->data['x_card_num'] = str_replace(' ', '', $this->request->post['cc_number']);
		$this->data['x_exp_date'] = $this->request->post['cc_expire_date_month'] . $this->request->post['cc_expire_date_year'];
		$this->data['x_card_code'] = $this->request->post['cc_cvv2'];
		$this->data['x_invoice_num'] = $this->session->data['order_id'];
		$this->data['x_solution_id'] = 'A1000015';
		...
		if ($order_info['shipping_method']) {
			...
			$this->data['x_ship_to_first_name'] = html_entity_decode($order_info['payment_firstname'], ENT_QUOTES, 'UTF-8');
			$this->data['x_ship_to_last_name'] = html_entity_decode($order_info['payment_lastname'], ENT_QUOTES, 'UTF-8');
			$this->data['x_ship_to_company'] = html_entity_decode($order_info['payment_company'], ENT_QUOTES, 'UTF-8');
			$this->data['x_ship_to_address'] = html_entity_decode($order_info['payment_address_1'], ENT_QUOTES, 'UTF-8') . ' ' . html_entity_decode($order_info['payment_address_2'], ENT_QUOTES, 'UTF-8');
			$this->data['x_ship_to_city'] = html_entity_decode($order_info['payment_city'], ENT_QUOTES, 'UTF-8');
			$this->data['x_ship_to_state'] = html_entity_decode($order_info['payment_zone'], ENT_QUOTES, 'UTF-8');
			$this->data['x_ship_to_zip'] = html_entity_decode($order_info['payment_postcode'], ENT_QUOTES, 'UTF-8');
			$this->data['x_ship_to_country'] = html_entity_decode($order_info['payment_country'], ENT_QUOTES, 'UTF-8');
		}

		if ($this->config->get('authorizenet_aim_mode') == 'test') {
			$this->data['x_test_request'] = 'true';
		}
