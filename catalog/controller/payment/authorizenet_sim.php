...
class ControllerPaymentAuthorizeNetSim extends Controller {
	public function index() {
		...
		$this->data['x_test_request'] = $this->config->get('authorizenet_sim_mode');
		...
		$this->data['x_fp_hash'] = hash_hmac('md5', $this->data['x_login'] . '^' . $this->data['x_fp_sequence'] . '^' . $this->data['x_fp_timestamp'] . '^' . $this->data['x_amount'] . '^' . $this->data['x_currency_code'], $this->config->get('authorizenet_sim_transaction_key'));

		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/authorizenet_sim.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/payment/authorizenet_sim.tpl', $this->data);
		} else {
			return $this->load->view('default/template/payment/authorizenet_sim.tpl', $this->data);
		}
	}
