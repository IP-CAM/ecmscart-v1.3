...
class ControllerPaymentPPPayflowIframe extends Controller {
	public function index() {
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/pp_payflow_iframe.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/payment/pp_payflow_iframe.tpl', $this->data);
		} else {
			return $this->load->view('default/template/payment/pp_payflow_iframe.tpl', $this->data);
		}
	}
	...
	public function paymentReturn() {
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/pp_payflow_iframe_return.tpl')) {
			$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/payment/pp_payflow_iframe_return.tpl', $this->data));
		} else {
			$this->response->setOutput($this->load->view('default/template/payment/pp_payflow_iframe_return.tpl', $this->data));
		}
	}
	...
	public function paymentIpn() {
		...
		if ($order_id) {
			...
			if ($order_info['order_status_id'] == 0 && $response_params['RESULT'] == '0' && $this->request->post['RESULT'] == 0) {
				...
				$this->model_payment_pp_payflow_iframe->updateOrder($this->data);
