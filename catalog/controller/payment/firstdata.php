...
class ControllerPaymentFirstdata extends Controller {
	public function index() {
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/firstdata.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/payment/firstdata.tpl', $this->data);
		} else {
			return $this->load->view('default/template/payment/firstdata.tpl', $this->data);
		}
	}
			...
			if ($local_hash == $this->request->post['notification_hash']) {
				...
				if ($this->request->post['txntype'] == 'preauth' || $this->request->post['txntype'] == 'sale') {
					if (isset($this->request->post['approval_code'])) {
						...
						$card_types = array(
							'M'         => $this->data['text_card_type_m'],
							'V'         => $this->data['text_card_type_v'],
							'C'         => $this->data['text_card_type_c'],
							'A'         => $this->data['text_card_type_a'],
							'MA'        => $this->data['text_card_type_ma'],
							'MAESTROUK' => $this->data['text_card_type_mauk']
						);
						...
						if ($response_parts[0] == 'Y') {
							if (isset($response_parts[3])) {
								...
								$message .= $this->data['text_address_response'] . $address_codes[$address_pass] . '<br />';
								$message .= $this->data['text_card_code_verify'] . $cvv_codes[$cvv_pass] . '<br />';
								$message .= $this->data['text_response_code_full'] . $this->request->post['approval_code'] . '<br />';
								$message .= $this->data['text_response_code'] . $response_parts[1] . '<br />';

								if (isset($this->request->post['cardnumber'])) {
									$message .= $this->data['text_response_card'] . $this->request->post['cardnumber'] . '<br />';
								}

								if (isset($this->request->post['processor_response_code'])) {
									$message .= $this->data['text_response_proc_code'] . $this->request->post['processor_response_code'] . '<br />';
								}

								if (isset($this->request->post['refnumber'])) {
									$message .= $this->data['text_response_ref'] . $this->request->post['refnumber'] . '<br />';
								}

								if (isset($this->request->post['paymentMethod'])) {
									$message .= $this->data['text_response_card_type'] . $card_types[strtoupper($this->request->post['paymentMethod'])] . '<br />';
								}
							}
							...
							$message .= $this->data['text_response_code_full'] . $this->request->post['approval_code'];
	...
	public function fail() {
		...
		if (isset($this->request->post['fail_reason']) && !empty($this->request->post['fail_reason'])) {
			...
			$this->session->data['error'] = $this->data['error_failed'];
		}
