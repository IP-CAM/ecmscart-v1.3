...
class ControllerPaymentRealexRemote extends Controller {
	public function index() {
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/realex_remote.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/payment/realex_remote.tpl', $this->data);
		} else {
			return $this->load->view('default/template/payment/realex_remote.tpl', $this->data);
		}
	}
			...
			if ($this->request->post['cc_type'] == 'visa' || $this->request->post['cc_type'] == 'mc' || $this->request->post['cc_type'] == 'amex') {
				...
				if (isset($verify_3ds->result) && $verify_3ds->result == '110' && isset($verify_3ds->enrolled) && $verify_3ds->enrolled == 'U') {
					if ($this->config->get('realex_remote_liability') != 1) {
						...
						$json['error'] = $this->data['error_3d_unable'];
		...
		if (isset($this->session->data['order_id'])) {
			...
			if ($signature_result->result == '00' && (strtoupper($signature_result->threedsecure->status) == 'Y' || strtoupper($signature_result->threedsecure->status) == 'A')) {
				...
				if ($this->config->get('realex_remote_liability') != 1) {
					...
					$message = $this->data['error_3d_unsuccessful'];
					$message .= '<br /><strong>' . $this->data['text_eci'] . ':</strong> (' . $eci . ') ' . $this->data['text_3d_s' . (int)$eci_ref];
					$message .= '<br /><strong>' . $this->data['text_timestamp'] . ':</strong> ' . (string)strftime("%Y%m%d%H%M%S");
					$message .= '<br /><strong>' . $this->data['text_order_ref'] . ':</strong> ' . (string)$md['order_ref'];

					if ($this->config->get('realex_remote_card_data_status') == 1) {
						$message .= '<br /><strong>' . $this->data['entry_cc_type'] . ':</strong> ' . (string)$md['cc_type'];
						$message .= '<br /><strong>' . $this->data['text_last_digits'] . ':</strong> ' . (string)substr($md['cc_number'], -4);
						$message .= '<br /><strong>' . $this->data['entry_cc_expire_date'] . ':</strong> ' . (string)$md['cc_expire'];
						$message .= '<br /><strong>' . $this->data['entry_cc_name'] . ':</strong> ' . (string)$md['cc_name'];
					}

					$this->model_payment_realex_remote->addHistory($md['order_id'], $this->config->get('realex_remote_order_status_decline_id'), $message);

					$this->session->data['error'] = $this->data['error_3d_unsuccessful'];
