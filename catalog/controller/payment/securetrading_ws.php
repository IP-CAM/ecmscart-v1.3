...
class ControllerPaymentSecureTradingWs extends Controller {
	...
	public function index() {
		...
		$this->data = $this->language->load('payment/securetrading_ws');
		...
		if ($order_info) {
			...
			foreach ($this->config->get('securetrading_ws_cards_accepted') as $card_type) {
				$this->data['cards'][$card_type] = $cards[$card_type];
			}

			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/securetrading_ws.tpl')) {
				return $this->load->view($this->config->get('config_template') . '/template/payment/securetrading_ws.tpl', $this->data);
			} else {
				return $this->load->view('default/template/payment/securetrading_ws.tpl', $this->data);
			}
		}
	}
				...
				if ($response !== False) {
					...
					if ($response_xml->response['type'] == 'THREEDQUERY') {
						...
						if ($error_code == 0) {
							...
							$json['message'] = $this->data['text_transaction_declined'];
							$json['status'] = 0;
						}
					} else {
						$json['message'] = $this->data['text_transaction_failed'];
						$json['status'] = 0;
					}
				} else {
					$json['message'] = $this->data['text_connection_error'];
					$json['status'] = 0;
				}
			} else {
				...
				if ($response) {
					...
					if ($error_code == 0) {
						...
						$status_code_mapping = array(
							0 => $this->data['text_not_given'],
							1 => $this->data['text_not_checked'],
							2 => $this->data['text_match'],
							4 => $this->data['text_not_match'],
						);

						$threed_status_mapping = array(
							'Y' => $this->data['text_authenticated'],
							'N' => $this->data['text_not_authenticated'],
							'A' => $this->data['text_authentication_not_completed'],
							'U' => $this->data['text_unable_to_perform'],
						);

						$message = sprintf($this->data['text_auth_code'], $authcode) . "\n";
						$message .= sprintf($this->data['text_postcode_check'], $status_code_mapping[$postcode_status]) . "\n";
						$message .= sprintf($this->data['text_security_code_check'], $status_code_mapping[$security_code_status]) . "\n";
						$message .= sprintf($this->data['text_address_check'], $status_code_mapping[$address_status]) . "\n";
						$message .= sprintf($this->data['text_3d_secure_check'], $threed_status_mapping[$threed_status]) . "\n";
						...
						$this->session->data['error'] = $this->data['text_transaction_declined'];
						$this->response->redirect($this->url->link('checkout/checkout', '', 'SSL'));
					}
				} else {
					$this->session->data['error'] = $this->data['error_failure'];
					$this->response->redirect($this->url->link('checkout/checkout', '', 'SSL'));
				}
			} else {
				$this->session->data['error'] = $this->data['error_failure'];
