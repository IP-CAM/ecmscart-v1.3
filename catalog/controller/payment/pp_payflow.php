...
class ControllerPaymentPPPayflow extends Controller {
	public function index() {
		$this->data = $this->language->load('payment/pp_payflow');
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/pp_payflow.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/payment/pp_payflow.tpl', $this->data);
		} else {
			return $this->load->view('default/template/payment/pp_payflow.tpl', $this->data);
		}
	}
	...
	public function send() {
		...
		if ($response_info['RESULT'] == '0') {
			...
			switch ($response_info['RESULT']) {
				case '1':
				case '26':
					$json['error'] = $this->data['error_config'];
					break;
				case '7':
					$json['error'] = $this->data['error_address'];
					break;
				case '12':
					$json['error'] = $this->data['error_declined'];
					break;
				case '23':
				case '24':
					$json['error'] = $this->data['error_invalid'];
					break;
				default:
					$json['error'] = $this->data['error_general'];
					break;
			}
		}
