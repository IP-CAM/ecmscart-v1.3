...
class ControllerModulePPLogin extends Controller {
	...
	public function index() {
		if (!$this->customer->isLogged()) {
			$this->data['client_id'] = $this->config->get('pp_login_client_id');
			$this->data['return_url'] = $this->url->link('module/pp_login/login', '', 'SSL');
			...
			if ($this->config->get('pp_login_button_colour') == 'grey') {
				$this->data['button_colour'] = 'neutral';
			} else {
				$this->data['button_colour'] = '';
			}
			...
			foreach ($languages as $language) {
				if ($language['status'] && ($language['code'] == $this->session->data['language']) && isset($locale[$language['language_id']])) {
					$this->data['locale'] = $locale[$language['language_id']];
				}
			}
			...
			$this->data['scopes'] = implode(' ', $scopes);

			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/pp_login.tpl')) {
				return $this->load->view($this->config->get('config_template') . '/template/module/pp_login.tpl', $this->data);
			} else {
				return $this->load->view('default/template/module/pp_login.tpl', $this->data);
			}
		}
	}
