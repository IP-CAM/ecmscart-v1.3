...
class ControllerModuleAmazonButton extends Controller {
	public function index($setting) {
		...
		if ($status) {			
			...
			$this->data['align'] = $setting['align'];

			$this->data['module'] = $module++;

			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/amazon_button.tpl')) {
				return $this->load->view($this->config->get('config_template') . '/template/module/amazon_button.tpl', $this->data);
			} else {
				return $this->load->view('default/template/module/amazon_button.tpl', $this->data);
			}
		}
	}
}