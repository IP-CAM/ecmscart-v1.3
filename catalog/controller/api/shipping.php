...
class ControllerApiShipping extends Controller {
	public function address() {
		$this->data = $this->load->language('api/shipping');
		...
		if ($this->cart->hasShipping()) {
			if (!isset($this->session->data['api_id'])) {
				...
				if (!$json) {
					...
					$json['success'] = $this->data['text_address'];
		...
		if (!isset($this->session->data['api_id'])) {
			...
			if (!$json) {
				...
				if ($json['shipping_methods']) {
					...
					$json['error'] = $this->data['error_no_shipping'];
				}
			}
		} else {
			$json['shipping_methods'] = array();
		...
		if (!isset($this->session->data['api_id'])) {
			...
			if ($this->cart->hasShipping()) {
				...
				if (empty($this->session->data['shipping_methods'])) {
					...
					$json['error'] = $this->data['error_method'];
				} else {
				...
				if (!$json) {
					...
					$json['success'] = $this->data['text_method'];
				}
			} else {
				unset($this->session->data['shipping_address']);
				unset($this->session->data['shipping_method']);
