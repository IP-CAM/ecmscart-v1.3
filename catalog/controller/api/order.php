...
class ControllerApiOrder extends Controller {
	public function add() {
		$this->data = $this->load->language('api/order');
		...
		if (!isset($this->session->data['api_id'])) {
			...
			if (!isset($this->session->data['customer'])) {
				$json['error'] = $this->data['error_customer'];
			}

			// Payment Address
			if (!isset($this->session->data['payment_address'])) {
				$json['error'] = $this->data['error_payment_address'];
			}

			// Payment Method
			if (!isset($this->session->data['payment_method'])) {
				$json['error'] = $this->data['error_payment_method'];
			}
			...
			if ($this->cart->hasShipping()) {
				...
				if (!isset($this->session->data['shipping_address'])) {
					$json['error'] = $this->data['error_shipping_address'];
				}

				// Shipping Method
				if (!isset($this->request->post['shipping_method'])) {
					$json['error'] = $this->data['error_shipping_method'];
				}
			} else {
			...
			if ((!$this->cart->hasProducts() && empty($this->session->data['vouchers'])) || (!$this->cart->hasStock() && !$this->config->get('config_stock_checkout'))) {
				$json['error'] = $this->data['error_stock'];
			}
			...
			foreach ($products as $product) {
				...
				if ($product['minimum'] > $product_total) {
					$json['error'] = sprintf($this->data['error_minimum'], $product['name'], $product['minimum']);
			...
			if (!$json) {
				...
				$order_status_id = $this->request->post('order_status_id',$this->config->get('config_order_status_id'));
		...
		if (!isset($this->session->data['api_id'])) {
			...
			if ($order_info) {
				...
				if (!$json) {
					...
					$order_data['payment_method'] = (isset($this->session->data['payment_method']['code']) ? $this->session->data['payment_method']['code'] : '');
					...
					if ($this->cart->hasShipping()) {
						...
						$order_data['shipping_code'] = (isset($this->session->data['shipping_code']['code']) ? $this->session->data['shipping_code']['code'] : '');
				...
				$json['error'] = $this->data['error_not_found'];
			}
		}
