...
class ControllerOpenbayAmazon extends Controller {
	public function order() {
		...
		$this->data = $this->language->load('openbay/amazon_order');
		...
		$order = array(
			...
			'payment_method' => $this->data['text_paid_amazon'],
			...
			'totals' => array(
				array(
					'code' => 'sub_total',
					'title' => $this->data['text_total_sub'],
					'value' => sprintf('%.4f', $products_total),
					'sort_order' => '1',
				),
				array(
					'code' => 'shipping',
					'title' => $this->data['text_total_shipping'],
					'value' => sprintf('%.4f', $products_shipping),
					'sort_order' => '3',
				),
				array(
					'code' => 'tax',
					'title' => $this->data['text_tax'],
					'value' => sprintf('%.4f', $products_tax),
					'sort_order' => '4',
				),
				array(
					'code' => 'shipping_tax',
					'title' => $this->data['text_total_shipping_tax'],
					'value' => sprintf('%.4f', $products_shipping_tax),
					'sort_order' => '6',
				),
				array(
					'code' => 'gift_wrap',
					'title' => $this->data['text_total_giftwrap'],
					'value' => sprintf('%.4f', $gift_wrap),
					'sort_order' => '2',
				),
				array(
					'code' => 'gift_wrap_tax',
					'title' => $this->data['text_total_giftwrap_tax'],
					'value' => sprintf('%.4f', $gift_wrap_tax),
					'sort_order' => '5',
				),
				array(
					'code' => 'total',
					'title' => $this->data['text_total'],
	...
	public function listing() {
		...
		$this->data = json_decode($decrypted, 1);

		$logger->write("Received data: " . print_r($this->data, 1));

		if ($this->data['status']) {
			$logger->write("Updating " . $this->data['product_id'] . ' from ' . $this->data['marketplace'] . ' as successful');
			$this->model_openbay_amazon_listing->listingSuccessful($this->data['product_id'], $this->data['marketplace']);
			$this->model_openbay_amazon_product->linkProduct($this->data['sku'], $this->data['product_id']);
			$logger->write("Updated successfully");
		} else {
			$logger->write("Updating " . $this->data['product_id'] . ' from ' . $this->data['marketplace'] . ' as failed');
			$this->model_openbay_amazon_listing->listingFailed($this->data['product_id'], $this->data['marketplace'], $this->data['messages']);
			$logger->write("Updated successfully");
		}
	}
	...
	public function listingReport() {
		...
		if ($this->data) {
			$this->model_openbay_amazon_product->addListingReport($this->data);
		}
	...
	public function product() {
		...
		$this->data = $this->openbay->amazon->decryptArgs($this->request->post['data']);
		...
		$decoded_data = (array)json_decode($this->data);
	...
	public function dev() {
		...
		$this->data_xml = simplexml_load_string($this->data);

		if(!isset($this->data_xml->action)) {
			$this->response->setOutput("error 004");
			return;
		}

		$action = trim((string)$this->data_xml->action);

		if ($action === "get_amazon_product") {
			if(!isset($this->data_xml->product_id)) {
				$this->response->setOutput("error 005");
				return;
			}
			$product_id = trim((string)$this->data_xml->product_id);
			if ($product_id === "all") {
